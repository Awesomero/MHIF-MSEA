...
def read_directed_graph(X, weighted):
    ...
    # the weights of redundant rwr_edges will be summed (by default in csr_matrix)
    A = csr_matrix((data, (rows, cols)), shape=(n, n))

    if not weighted:
        # no redundant rwr_edges are allowed for unweighted graphs
        I, J, K = find(A)
...
def read_undirected_graph(X, weighted):
    ...
    # this is under the assumption that src_id <= dst_id for all rwr_edges (see line 80 in this code)
    A = _A + _A.T
...
def read_graph(path, graph_type):
    ...
    if min_weight < 0:
        raise ValueError('Negative edge_dis weights')
